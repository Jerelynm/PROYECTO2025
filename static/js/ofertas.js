document.addEventListener('DOMContentLoaded', () => {
  ...
  const btn = document.getElementById('themeBtn');     // <- botón
  const icon = document.getElementById('themeIcon');   // <- ícono dentro del botón
  ...
  document.querySelectorAll('.btn-cart').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      btn.style.background = 'var(--discount)';
      setTimeout(() => { btn.textContent = 'Añadir al carrito'; btn.style.background = 'var(--secondary)'; }, 1800);
    });
  });
});
